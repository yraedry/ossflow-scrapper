"""Service-level tests for subtitle-generator FastAPI wrapper (real pipeline mocked)."""
...
def _fake_task(req: RunRequest, emit) -> None:
    emit(JobEvent(type="log", data={"message": "fake transcribe"}))
...
def test_events_streams_error_on_task_failure(tmp_path):
    def boom(req, emit):
        raise RuntimeError("whisperx unavailable")
