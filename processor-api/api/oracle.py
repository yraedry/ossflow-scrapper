...
async def _download_poster_if_missing(folder: Path, poster_url: str | None) -> str | None:
    """Download poster_url to folder/poster.<ext> if no local poster exists.

    Returns the saved filename, or None if skipped/failed.
    """
    if not poster_url or _has_local_poster(folder):
        return None
    try:
