...
class ScrapeChapter(BaseModel):
    title: str = Field(min_length=1, max_length=300)
    start_s: float = Field(ge=0)
    end_s: float = Field(gt=0)

    @model_validator(mode="after")
    def _end_after_start(self) -> "ScrapeChapter":
...
class ScrapeVolume(BaseModel):
    number: int = Field(ge=1, le=50)
    chapters: list[ScrapeChapter] = Field(min_length=1)
    total_duration_s: float = Field(gt=0)

    @field_validator("chapters")
    @classmethod
    def _monotonic_starts(
        cls, v: list[ScrapeChapter]
    ) -> list[ScrapeChapter]:
...
class ScrapeResult(BaseModel):
    ...
    volumes: list[ScrapeVolume] = Field(min_length=1)

    @field_validator("volumes")
    @classmethod
    def _unique_volume_numbers(
        cls, v: list[ScrapeVolume]
    ) -> list[ScrapeVolume]:
    ...
    def volume(self, number: int) -> Optional[ScrapeVolume]:
